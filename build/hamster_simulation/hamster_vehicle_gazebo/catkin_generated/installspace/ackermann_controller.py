...
class _AckermannCtrlr(object):
    ...
    def __init__(self):
        ...
        list_ctrlrs = rospy.ServiceProxy("controller_manager/list_controllers",
                                         ListControllers)
        # rospy.logwarn("Waiting for controller_manager/list_controllers service...")
        ...
        # rospy.logwarn("Searching for shock absorbers")
        # rospy.logwarn("Namespace = ", rospy.get_namespace())
        # rospy.logwarn("shock_absorbers list count = ", len(shock_param_list))
        ...
        try:
            # rospy.logwarn("Getting tf_prefix...")
            self._tf_prefix = str(rospy.get_param("~tf_prefix", ""))
        except:
            self._tf_prefix = "ackermann_vehicle"

        # Publishing frequency
        try:
            rospy.logwarn("Getting publishing_frequency...")
        ...
        # divided by two.
        rospy.logwarn("Creating tf listener")
        tfl = tf.TransformListener()
        rospy.logwarn("Getting link pos left_steer_link_name")
        ls_pos = self._get_link_pos(tfl, left_steer_link_name)
        # rospy.logwarn("Getting link pos left_steer_link_name done")
        rospy.logwarn("Getting link pos right_steer_link_name")
        rs_pos = self._get_link_pos(tfl, right_steer_link_name)
        rospy.logwarn("Getting link pos right_steer_link_name done")
        ...
        rospy.logwarn("Subscribing to ackermann_cmd...")
    ...
    def _get_link_pos(self, tfl, link):
        ...
        # link = self._tf_prefix + "/" + str(link)
        # rospy.logwarn("Waiting for tf " + str(link))

        while not rospy.is_shutdown():
            try:
                trans, not_used = \
                    tfl.lookupTransform(self._tf_prefix + "/" + self._right_rear_link_name, self._tf_prefix + "/" + link, 
                                        rospy.Time(0))
                # rospy.logwarn("tf received")
...
def _wait_for_ctrlr(list_ctrlrs, ctrlr_name):
    ...
    # rospy.logwarn("Waiting for controller ", ctrlr_name, "...")
    while not rospy.is_shutdown():
